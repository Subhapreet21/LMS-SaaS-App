...
// NEW ROUTE: /api/ping
// Moved from /api/cron to bypass Vercel/UptimeRobot caching issues
...
async function handleRequest() {
    try {
        ...
        if (error) {
            ...
            // Return 200 even on DB error to keep Uptime Robot happy (we check logs for errors)
            // Or return 500 if you want alerts. Let's return 200 to avoid false positive 'down' 
            // if it's just a momentary network blip, but log it.
